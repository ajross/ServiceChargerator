    ...
    loadData() {
        return new Promise<void> ((resolve, reject) => {
          Papa.parse(this.csvFilePath, {
            ...
            header: true,
            complete: (result) => {
                this.data = (result.data as ChargeErrorData[]).map(row => {
                    for (let key in row) {
                        if ((row as any)[key].toLowerCase() === 'true') {
                            (row as any)[key] = true;
                        } else if ((row as any)[key].toLowerCase() === 'false') {
                            (row as any)[key] = false;
                        }
                    }
                    return row;
                });
